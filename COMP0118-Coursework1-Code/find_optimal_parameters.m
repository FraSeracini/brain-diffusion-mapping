function [best_parameters, min_RESNORM, RESNORM_per_start_point] = find_optimal_parameters(Model_Name, startx, num_runs, Avox, bvals, qhat)
% This function runs a non-linear optimization process multiple times from different
% randomized starting points to find the global minimum of the objective function
% for the Ball-and-Stick model.
%
% INPUTS:
%   - Model_Name : Name of the objective function to be minimized.
...
%   - S0 (baseline signal) is positive.
%   - Diffusivity (d) is positive.
%   - Volume fraction (f) is constrained between 0 and 1.
%   - Multiple random perturbations are used to explore different initial conditions.
...
for n = 1 : num_runs
    ...
    % Now run the fitting
    try
        [parameter_hat, RESNORM, ~, ~] = fminunc(Model_Name, startx_n, h, Avox, bvals, qhat);
    catch
        continue;
    end
