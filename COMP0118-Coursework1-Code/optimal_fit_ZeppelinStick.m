function [best_parameters, min_RESNORM, RESNORM_per_start_point] = optimal_fit_ZeppelinStick(startx, num_runs, Avox, bvals, qhat)
% This function is optimized for the data set used in sections 1.3 and 1.4
%
% This function performs multiple optimization runs to identify the global minimum
% of the objective function for the Zeppelin-and-Stick diffusion model.
%
% INPUTS:
...
for n = 1 : num_runs
    ...
    startx_n(6) = startx(6) + randn*1e-03;
    ...
    % Impose that 0 <= s <= 1
    while startx_n(6) < 0 || startx_n(6) >1
        startx_n(6) = startx(6) + randn*1e-01;
    end
    ...
    startx_n(6) = sqrt(-log(startx_n(6)));
    ...
    % Now run the fitting
    try
        [parameter_hat, RESNORM, ~, ~] = fminunc('ZeppelinStickSSD', startx_n, h, Avox, bvals, qhat);
    catch
        continue;
    end
    ...
    parameter_hat(6) = exp(-parameter_hat(6)^2);
